...
const newservicesData = [
    ...
    details: {
      ...
      sections: [
        ...
        },
        {
          ...
          list: ['Housing the underprivileged', 'Developing new housing for Australians'],
        },
        {
          key: 'message',
          title: null,
...

  {
    ...
    demandText: "In-line with Russia, China, Venezuela, Turkey and the list continues.",
...
async function seed() {
  try {
    ...
    const existingIds: string[] = await client.fetch(`*[_type == "services"]._id`);
    ...
    for (let i = 0; i < newservicesData.length; i++) {
      ...
      const slugSource = link.replace(/^\//, '') || item.title || `service-${i}`;
      ...
      if (Array.isArray(doc.references) && doc.references.length === 0) delete doc.references;
